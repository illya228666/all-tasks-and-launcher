...
internal sealed class PetExplorer
{
    ...
    private int _phase; // 0 rest, 1 approach, 2 grab, 3 climb, 4 pull, 5 flight, 6 pickup
    ...
    internal void Update(PetState state, PetEnvironment env, HatWorld hat, PetBehavior behavior, PetSpeech speech, long now, float elapsed)
    {
        ...
        if (_phase == 5)
        {
            ...
            if (landing is not null) Land(landing, state, behavior, speech, now);
            else if (_footY >= scene.Size.Height) Land(available["ruin:floor"], state, behavior, speech, now);
            else { state.Mode = PetMode.Traversing; state.Row = PetAnimationCatalog.JumpRow; state.Frame = _vy < 0 ? 1 : 3; }
        }
