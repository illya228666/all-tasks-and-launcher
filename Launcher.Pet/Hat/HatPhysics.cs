...
internal sealed class HatPhysics
{
    ...
    internal void Advance(HatState state, float elapsedSeconds, float scale = 1)
    {
        ...
        float dx = SwayDistance * scale * (MathF.Cos(previousPhase) - MathF.Cos(phase)
        ...
        float targetSpeed = MaxSpeed * scale * (1f - GlideLift * swing * swing);
