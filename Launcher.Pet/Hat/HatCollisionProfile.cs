...
/// </summary>
public sealed class HatCollisionProfile
{
    ...
    public IReadOnlyList<HatCollisionSegment> Segments => _segments;
    public IReadOnlyList<HatCollisionConnector> Connectors => _connectors;
    ...
    public HatCollisionProfile(Size hatSize)
    {
        ...
        _connectors = new[]
        {
            new HatCollisionConnector(sideWidth, centerContactY, brimContactY, _segments[0]),
            new HatCollisionConnector(hatSize.Width - sideWidth, centerContactY, brimContactY, _segments[2])
        };
    }

    public HatCollision? FindFirstCollision(IEnumerable<HatSurface> surfaces, RectangleF previousHatBounds, RectangleF currentHatBounds, bool resolveInitialOverlap)
    {
        ...
        foreach (HatSurface surface in surfaces)
        {
            ...
            if (!resolveInitialOverlap)
                continue;
            HatCollisionSegment? connectorCollision = FindConnectorCollision(surface.Bounds, previousHatBounds);
            if (connectorCollision is null)
                continue;
            float lift = previousHatBounds.Top + connectorCollision.Value.ContactY - surface.Bounds.Top;
    ...
    private HatCollisionSegment? FindConnectorCollision(Rectangle surfaceBounds, RectangleF hatBounds)
    {
    ...
    public static bool HorizontallyOverlaps(Rectangle surfaceBounds, float hatLeft, HatCollisionSegment segment)
    {
