...
internal sealed class DevicePanelConnection : IDisposable
{
    ...
    private void StateChanged(DeviceState state) => _post(() => _window.Device.Display(state.IsConnected, state.PortName));
    private void IndicatorCompleted(bool enabled, bool available) => _post(() => _window.Status.ShowHint(!available ? "Controller nicht erreichbar." : enabled ? "LED eingeschaltet." : "LED ausgeschaltet."));
