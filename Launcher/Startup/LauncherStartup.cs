...
internal static class LauncherStartup
{
    internal static LauncherSession Create()
    {
        ...
        try
        {
            pet = new PetWindowsSession(window, window.Apps.PetArea, new PetWorld(new Random()));
        }
        ...
        string warning = string.Join(" | ", new[] { read.Warning, petWarning }.Where(text => !string.IsNullOrWhiteSpace(text)));
