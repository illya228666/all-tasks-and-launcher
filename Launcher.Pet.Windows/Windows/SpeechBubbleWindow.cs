...
internal sealed class SpeechBubbleWindow : TransparentOverlayWindow
{
    ...
    internal void Display(Point head, int visibleLetters, Color surface, Color text, Color border)
    {
        ...
        var location = new Point(Math.Clamp(x, workArea.Left, Math.Max(workArea.Left, workArea.Right - _bubbleSize.Width)), Math.Clamp(y, workArea.Top, Math.Max(workArea.Top, workArea.Bottom - _bubbleSize.Height)));
        ...
        bool redraw = _paintedLetters != visibleLetters || _onRight != onRight || _tailY != tailY || _surface != surface || _text != text || _border != border;
        if (redraw)
        {
    ...
    private void DrawBubble()
    {
        ...
        for (int index = 0; index < _lines.Count; index++)
        {
            ...
            if (count > 0)
                graphics.DrawString(_phrase.Substring(start, count), _textFont, foreground, new PointF(left + TextPadding, TextPadding + index * _lineHeight), _textFormat);
        }
